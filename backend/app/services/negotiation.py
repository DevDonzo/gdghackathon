...
def _financials(bill: dict[str, Any], scenario: Scenario) -> dict[str, float]:
    ...
    if scenario.scenario_id == "happy_path":
        offer_one = _round_money(max(current - 15, current * 0.82))
        offer_two = _round_money(max(current - 33, current * 0.61))
        walk_away = _round_money(max(current - 22, current * 0.74))
        credit = 25.0 if fee_credit <= 0 else min(40.0, fee_credit + 10.0)
    elif scenario.scenario_id == "fee_recovery_path":
        offer_one = _round_money(max(current - 6, current * 0.93))
        offer_two = _round_money(max(current - 10, current * 0.88))
        walk_away = current
        credit = _round_money(min(max(fee_credit, 20.0), 45.0))
    else:
        offer_one = _round_money(max(current - 10, current * 0.88))
        offer_two = _round_money(max(current - 18, current * 0.79))
        walk_away = _round_money(max(current - 12, current * 0.85))
        credit = 0.0
...
def _plan_blueprints(bill: dict[str, Any], scenario: Scenario) -> tuple[list[dict[str, Any]], dict[str, float]]:
    ...
    if scenario.scenario_id == "happy_path":
        turns = [
            {"role": "negotiator", "intent": "probe", "objective": scenario.objective_labels[0], "facts": f"Monthly bill is ${money['current']:.2f}. Mention {primary_angle} on the {plan_name}."},
        ...
        turns = [
            {"role": "negotiator", "intent": "probe", "objective": scenario.objective_labels[0], "facts": f"Question the unexpected fee charges and cite the ${money['current']:.2f} bill."},
        ...
        turns = [
            {"role": "negotiator", "intent": "probe", "objective": scenario.objective_labels[0], "facts": f"Open with the ${money['current']:.2f} monthly total and explain the plan is no longer competitive."},
...
def create_negotiation_document(bill: dict[str, Any]) -> dict[str, Any]:
